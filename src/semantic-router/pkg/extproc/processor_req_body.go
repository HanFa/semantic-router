...
func (r *OpenAIRouter) handleAnthropicRouting(openAIRequest *openai.ChatCompletionNewParams, originalModel string, targetModel string, decisionName string, ctx *RequestContext) (*ext_proc.ProcessingResponse, error) {
	...
	// Reject streaming request before it is supported
	...
	// Start upstream span for tracing
	upstreamSpan := r.startAnthropicUpstreamSpan(targetModel, ctx)
	startTime := time.Now()
	...
	// End upstream span with latency and error (if any)
	r.endAnthropicUpstreamSpan(upstreamSpan, latency, err)
	...
	if ctx.ResponseAPICtx != nil && ctx.ResponseAPICtx.IsResponseAPIRequest && r.ResponseAPIFilter != nil {
		if translated, err := r.ResponseAPIFilter.TranslateResponse(ctx.TraceContext, ctx.ResponseAPICtx, responseBody); err != nil {
			logging.Errorf("Response API translation error for Anthropic response: %v", err)
		} else {
			finalBody = translated
		}
	}

	// Return immediate response with Anthropic's response (translated if Response API)
	return r.createJSONResponseWithBody(200, finalBody), nil
}
...
// startAnthropicUpstreamSpan starts an upstream span for Anthropic API calls.
// Returns the span which should be ended after the API call completes.
func (r *OpenAIRouter) startAnthropicUpstreamSpan(model string, ctx *RequestContext) trace.Span {
	if ctx.TraceContext == nil {
		return nil
	}
	_, span := tracing.StartSpan(ctx.TraceContext, tracing.SpanUpstreamRequest,
		trace.WithSpanKind(trace.SpanKindClient))
	tracing.SetSpanAttributes(span,
		attribute.String(tracing.AttrModelName, model),
		attribute.String(tracing.AttrModelProvider, "anthropic"),
		attribute.String(tracing.AttrEndpointAddress, "api.anthropic.com"))
	return span
}

// endAnthropicUpstreamSpan ends the upstream span with latency and optional error.
func (r *OpenAIRouter) endAnthropicUpstreamSpan(span trace.Span, latency time.Duration, err error) {
	if span == nil {
		return
	}
	tracing.SetSpanAttributes(span,
		attribute.Float64(tracing.AttrProcessingTimeMs, float64(latency.Milliseconds())))
	if err != nil {
